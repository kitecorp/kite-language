...
import io.kite.semantics.types.FunType;
...
import static io.kite.syntax.ast.generated.KiteParser.*;
 ...
 */
public class KiteASTBuilder extends io.kite.syntax.ast.generated.KiteParserBaseVisitor<Object> {
    ...
    public ReturnStatement visitReturnStatement(ReturnStatementContext ctx) {
        Expression arg = ctx.expression() != null ?
                ...
                TypeIdentifier.type(ValueType.Void);  // ✅ Default to void
    ...
    /**
     * Extract the string value from a stringLiteral context, handling both quoted forms.
     */
    private String extractStringValue(StringLiteralContext ctx) {
        if (ctx.SINGLE_STRING() != null) {
            String raw = ctx.SINGLE_STRING().getText();
            return raw.substring(1, raw.length() - 1);
        } else if (ctx.interpolatedString() != null) {
            // For contexts that expect a plain string (like import),
            // concatenate all text parts (interpolations would be an error)
            var parts = ctx.interpolatedString().stringPart();
            StringBuilder sb = new StringBuilder();
            for (var part : parts) {
                sb.append(getStringPartText(part));
            }
            return sb.toString();
        }
        return "";
    ...
    public Expression visitInterpolatedString(InterpolatedStringContext ctx) {
        ...
        if (!hasInterpolation) {
            ...
            return StringLiteral.string(sb.toString());
        }

        // Has interpolation - create StringInterpolation
        var interpolation = new StringInterpolation();
        var textBuffer = new StringBuilder();

        for (var part : parts) {
            if (part.INTERP_START() != null) {
    ...
    /**
     * Extract text from a stringPart (handles escape sequences).
     */
    private String getStringPartText(StringPartContext part) {
        if (part.STRING_TEXT() != null) {
            return part.STRING_TEXT().getText();
        } else if (part.STRING_ESCAPE() != null) {
            // Handle escape sequences like \n, \t, \\, \"
            String escaped = part.STRING_ESCAPE().getText();
            if (escaped.length() >= 2) {
                char c = escaped.charAt(1);
                return switch (c) {
                    case 'n' -> "\n";
                    case 't' -> "\t";
                    case 'r' -> "\r";
            ...
            return escaped;
        } else if (part.STRING_DOLLAR() != null) {
            return "$";
        }
        return "";
    }
